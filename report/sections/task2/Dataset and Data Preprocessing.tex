...
Figure~\ref{fig:visdrone_label_stats} shows several statistical visualizations of the training set annotations. The category distribution is noticeably imbalanced, with the \texttt{car} category containing the largest number of instances, while some rare categories such as \texttt{awning-tricycle} contain significantly fewer samples. The bounding box statistics further show that the dataset is dominated by small objects with diverse aspect ratios.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/task2/labels.jpg}
    \caption{Visualization statistics of the VisDrone training annotations, including category distribution, bounding box overlap visualization, center-point heatmap, and width-height heatmap.}
    \label{fig:visdrone_label_stats}
\end{figure}
